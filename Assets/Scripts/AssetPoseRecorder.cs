...
{   
    ...
    {
        DebugLogger.Instance.Log("ResetRecording in " + gameObject.name);
        isRecording = false;
        isPlayingBack = false;
        recordable.ResetData();
    ...
    private void Update()
    {
        if (isRecording)
        {
            DebugLogger.Instance.Log("Current Position: " + recordable.playbackObject.transform.localPosition + " Last Recorded Position: " + lastLocalPosition);
            ...
            DebugLogger.Instance.Log("Distance: " + Vector3.Distance(recordable.playbackObject.transform.localPosition, lastLocalPosition));
            if (Vector3.Distance(recordable.playbackObject.transform.localPosition, lastLocalPosition) > 0.0001f)            
            {
                
                //print into debugger.instance.log both the current position and the last recorded position
                
                DebugLogger.Instance.Log("Added new frame data for " + gameObject.name + " at " + Time.time);
